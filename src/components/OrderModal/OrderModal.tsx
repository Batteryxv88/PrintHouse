...
const OrderModal: FC<OrderModalProps> = ({ isOpen, onClose, productName }) => {
    if (!isOpen) return null;

    const handleWhatsAppClick = () => {
        const phoneNumber = '+79153987112';
        const message = encodeURIComponent(`Добрый день! Мне нужно сделать заказ ${productName}.`);
    ...
    const handleEmailClick = () => {
        ...
        const subject = encodeURIComponent(`Заказ ${productName}`);
        const body = encodeURIComponent(`Добрый день! Мне нужно сделать заказ ${productName}.`);
    ...
    return (
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
                ...
                <h2 className={styles.modalTitle}>Выберите способ оформления заказа</h2>
